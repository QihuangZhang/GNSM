...
plot.selectgraph = function(x, ...){
  ...
  if(!x$cov.input)
  {
    ...
    plot(rev(out.select$lambda), rev(x$sparsity), xlab = expression(paste("Regularization Parameter ",lambda)), ylab = "Sparsity Level", type = "l",xlim = range(x$lambda))
...
Tablelow0.72 <- SensiAnalysis(-2.944,0.72) 
Tablemedian0.72 <- SensiAnalysis(-2.197,0.72) 
Tablehigh0.72 <- SensiAnalysis( -1.386,0.72) 

Table1 <- integrate(Tablelow0.72, Tablemedian0.72, Tablehigh0.72)
...
Table2 <- integrate(Tablelow0.77, Tablemedian0.77, Tablehigh0.77)
xtable(Table2,digits = 3)

# Higher Measurement Error

Tablelow0.82 <- SensiAnalysis(-2.944,0.82) 
Tablemedian0.82 <- SensiAnalysis(-2.197,0.82) 
Tablehigh0.82 <- SensiAnalysis( -1.386,0.82) 

Table3 <- integrate(Tablelow0.82, Tablemedian0.82, Tablehigh0.82)
