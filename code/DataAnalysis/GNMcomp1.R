...
DesMatrix <- (genotypes[,3:dim(genotypes)[2]])
...
Covariates1 <- Covariates2 <- as.matrix(cbind(Intercept=rep(1,length(Covariates_all[,2])),Covariates_all[,! names(Covariates_all) %in% c("Y1star","Y2star","BW")])) 
...
## 2.2.1 Naive model ####
naive.model1 <- lm(Y1star ~  ., data = data.frame(completedata[,-2]))
naive.model2 <- glm(Y2star ~ ., family = binomial(link = logit), data = data.frame(completedata[,-1]))
...
SensiAnalysis <- function(alphasensi,sigma_esensi){
  ...
  Table_numeric <- data.frame(SNPnames = c(rep(c("Intercept",SNPlist[-1*1:2],"v1","v2","v3","c4"),2),"sigma","phi"),
                              propobeta=betahat,
