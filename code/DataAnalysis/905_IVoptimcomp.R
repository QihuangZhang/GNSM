...
GEE_covIV <- function(theta, data.validation, data.mismeasure, Weight,
  ...
  M0 <- GEE_GAMMA_IV0(theta, 
                      ...
                      CovMis2 = as.matrix(data.validation[,11:12]), 
                      gamma1, gamma, alpha1, alpha0, sigma_e,
                      fixgamma1,fixgamma,fixsigma_e,fixalpha1,fixalpha0)
  ...
  M1 <- GEE_GAMMA_IVI(theta, 
                      ...
                      DesignMatrix2 = as.matrix(data.mismeasure[,3:6]), 
                      CovMis1 = as.matrix(data.mismeasure[,7:8]), 
                      CovMis2 = as.matrix(data.mismeasure[,9:10]), 
                      gamma1, gamma, alpha1, alpha0, sigma_e,
                      fixgamma1,fixgamma,fixsigma_e,fixalpha1,fixalpha0)
...
NR <- Weightestimate(intial2, omegas = omegas, 
                     ...
                     gamma1=1, gamma=c(gamma0,gamma2), alpha1=alpha1, alpha0=alpha0, sigma_e=sigma_e,
                     fixgamma1=1,fixgamma=c(0,0),fixsigma_e=0,fixalpha1=c(0,0),fixalpha0=c(0,0),
                     optimal = T)
...
Table_numeric <- data.frame(naivebeta=c(naivebeta,rep(0,9)),
                            sdnaive=c(sdnaive,rep(0,9)),
                            Z_naive=c(Zvalue_naive,rep(0,9)),
                            p_naive=c(pvalue_naive,rep(0,9)),
                            propobeta=betahat,
                            proposd = sd,
                            propoZ = Zvalue,
                            propop = pvalue)
